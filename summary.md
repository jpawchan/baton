...
| Locks and atomic state | `file_lock`, `task_lock`, `atomic_write`, `atomic_json`, `sync_directory`, and `lock_path`; atomic writes fsync the new file and parent directory, and writer order is scheduler before task/handoff. |
...
| Capsule and report parsing | `CAPSULE_SECTIONS`, `task_spec_sections`, `memory_index_entries`, `context_capsule_components` + `compile_context_capsule`, `stored_context_capsule_components`, `commonmark_fence_state`, and `report_section_problems`. |
...
├── archive/<id>.{json,md} + <id>.work/  terminal state, spec, launch evidence
└── .locks/                      scheduler, execution, memory, per-task, orchestrator-handoff
```

The cursor and archive journal are lazily created managed safety state even though `init` does not pre-create them. Do not hand-edit one handoff representation, manually move archive artifacts, or delete a pending journal; rerun `.baton/baton archive` for transaction recovery.
...
Read this before opening a durability, parsing, evidence, handoff, archive, validation, or performance investigation. It separates what hardening commit `eeb6894` fixed from the related scheduler/state-loading work in parent commit `28dc3c2`; rerun the named regressions before assuming the same symptom has a new cause.

| Prior failure mode | Root cause and completed fix | Start here when debugging |
| --- | --- | --- |
| An atomically replaced file could disappear after a crash, or a plain relative output path failed. | `eeb6894`: `atomic_write` now uses `.` for an empty dirname, fsyncs file bytes and the parent directory, closes descriptors, cleans temporary files, and propagates sync/replace failure. | `atomic_write`, `atomic_json`; search tests for `atomic_write_*parent`, `atomic_write_*relative`, and `atomic_write_*replace`. |
| Differently cased observed paths collapsed to one set element and passed changed-path attribution. | `eeb6894`: finalization/review require equal list cardinality as well as equal case-folded sets. | `finalize_task`, `review_evidence_details`, lifecycle validation; `test_case_colliding_observed_paths_fail_changed_path_attribution`. |
| A required report heading inside a fenced code block counted as real, or an unusual valid fence hid a real heading. | `eeb6894`: `commonmark_fence_state` models character, opening length, indentation, and backtick-info restrictions; `report_section_problems` routes only headings outside fences. | `commonmark_fence_state`, `report_section_problems`; search `test_report_*fence*`. |
...
| Memory index/body records drifted, duplicate headings passed, or structural-looking body text corrupted updates. | `eeb6894`: `memory_records` enforces one canonical Entries section, exact indexed/body parity, unique headings, valid metadata, and complete referenced bodies; memory add validates before mutation. | `memory_index_entries`, `memory_records`, `cmd_memory_add`; search `test_memory_*entries*`, `test_memory_*structural*`, and `test_missing_memory_body_*`. |
| `validate` accepted nonterminal archived state, missing review evidence, or split/orphaned archive companions. | `eeb6894`: shape-first task validation and `archive_layout_problems` aggregate read-only diagnostics for lifecycle evidence, terminal status, pending journals, and companion topology. Recovery remains in `cmd_archive`, never `validate`. | `task_validation_shape`, `task_problems`, `archive_layout_problems`, `cmd_validate`; search `test_validate_*archived*`, `test_validate_*review*`, and `test_validate_*companion*`. |
| Large task sets made scope scheduling and repeated state reads unnecessarily expensive. | `28dc3c2`, not `eeb6894`: `ScopeOverlapIndex` indexes literal prefixes conservatively and task creation reuses one active/archive load. `eeb6894` later made the benchmark fixture's archived review evidence realistic. | `ScopeOverlapIndex`, `pick_wave`, `cmd_task_create`, `tools/benchmark_performance.py`; `test_scope_overlap_index_matches_conservative_overlap_rules`, `test_task_create_loads_active_and_archive_once`, and `test_performance_fixture_has_valid_finalized_review_evidence`. |
| A worker inheriting a never-EOF stdin blocked before its first write and failed as `worker_timeout` with a zero-byte log. | The completed fix passes `stdin=subprocess.DEVNULL` at all three subprocess call sites and prints a never-started advisory after such a timeout. | Start at `run_one_worker`, `git_run`, `worker_left_no_evidence`, and `finalize_task`; then run `test_worker_does_not_inherit_orchestrator_stdin`, `test_run_one_worker_launches_the_worker_with_devnull_stdin`, `test_worker_timeout_without_evidence_reports_never_started`, and `test_worker_timeout_with_evidence_omits_never_started_advisory`. |
...
Late independent review reproduced two release-blocking defects: handoff consumption overflow and archive destination replacement at the move boundary. Their tracked regressions preserve the 3,989-character unconsumed budget/legacy compaction path and native no-replace archive primitive above. Do not revert either to a preflight-only check.
...
- `tools/benchmark_performance.py` emits measurements; it does not compare them with thresholds or independently declare a regression. Compare equivalent fixtures, enough samples, environment, and JSON shape.
...
- If validation reports a pending archive transaction, preserve the journal and both source/destination sides and rerun `archive`; validation is diagnostic-only. Manual moves or journal deletion can create an ambiguous topology that recovery intentionally refuses to mutate.
...
- All three Baton subprocess call sites — `run_one_worker` and both Git call sites (`working_tree_state`'s dirty-tree check and `git_run`) — pass `stdin=subprocess.DEVNULL`; a worker must never inherit the orchestrator's standard input, and passing a pipe or a TTY instead reintroduces the hang.
